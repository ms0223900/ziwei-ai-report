...
- [ ] S10-3：使用同一組欄位（**含空白、`()!*-_.~` 與中文**）與同一組 HashKey／HashIV，腳本匯出的函式結果與 `computeCheckMacValue` 相同
- [ ] 斷言可以產生 ReturnURL 與 PeriodReturnURL 兩種 payload（參數：`mtn` 必填、`totalSuccessTimes`、`rtnCode`、`gwsr`、`simulate`）
- [ ] 測試只 import 具名 export，不觸發 CLI 主流程
