...
- [x] US-002 次要區點數追問與訂閱入口

### Phase 2 — 預覽純邏輯

- [x] US-003 預覽態解析測試（預期紅燈；待實作轉綠）
- [x] US-004 預覽態解析實作
...
- 已掃描至：US-005（2026-09-12）
- 已知待觀察熱點：
  - `components/report/ReportCard.tsx`（US-001／002／005 皆改；中風險觀察，未達重構門檻）
  - `components/report/AdvancedLockedPanel.tsx`（同上；仍短）
- 備註：US-003／US-004 為純函式，未與畫面檔 churn 疊加。無反模式達重構門檻。低／中風險，暫不重構。
