...
- [x] US-006 點數包建單與閘門 測試（預期紅燈；待實作轉綠）
- [x] US-007 點數包建單與閘門 實作

### Phase 3 — Webhook 履約

- [x] US-008 履約分派與加點 測試（預期紅燈；待實作轉綠）
- [ ] US-009 履約分派與加點 實作
...
- 已掃描至：US-007（2026-09-22）
- 已知待觀察熱點：無
- 備註：US-006 只加 checkout 測試；US-007 只改閘門條件。沒有兩個任務改同一產品檔。低風險，暫不重構。
