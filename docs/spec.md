...
把「可計費的 AI 解讀」推進成單次解鎖、點數追問、訂閱三種收費模式,第一步先有訪客能跑完的解讀 MVP。基本與進階是**同一份 JSON 的兩個深度**,不是兩次互不參考的生成。介面與報告一律標示娛樂用途,不作為醫療、法律、財務、投資或重大人生決策依據。單元 1 畫面視覺見 [`docs/design-brief.md`](design-brief.md)。
